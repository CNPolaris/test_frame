...
class QuestionApi(BasicApi):
    ...
    def question_search(self):
        ...
        _response = api.request(api_item["method"], api_item["path"], api_item["headers"], api_item["body"],
                                api_item["params"],
                                api_item["expect"])
        ag.api_param["question_id"] = self.get_item_of_search(_response.json()["data"]["data"], "shortTitle", "测试", "id")
        # 公有4--断言判断接口是否真正请求成功
        assert _response.json()["code"] == api_item["expect"]["code"]
        return _response.json()
